...
FindArcCrossingPointsResult find_arc_crossing_points(PolygonInfo& a_info, PolygonInfo& b_info, Callbacks& callbacks)
{
  ...
  if (a_fwd_edge_side < 0 || (a_fwd_edge_side == 0 && cross_is_negative(a_fwd_edge.dir, b_perturbation_vector)))
  {
    ...
    {
      return FindArcCrossingPointsResult::disjoint;
    }
    ...
    a_is_inner = true;
    result = FindArcCrossingPointsResult::b_side_vertex_outside_a;
  }
  else
  {
    ...
    else
    {
      b_fwd_edge = arc_first_forward_edge<arc>(b_info);
      a_is_inner = false;
      result = FindArcCrossingPointsResult::b_side_vertex_inside_a;
    }
  }
...
{
  FindCrossingPointsResult result;
                                                                          ...
                                                                          arc_first_vertex_it<Arc::lower>(b_info)->x()))
  {
            ...
            arc_first_vertex_it<Arc::upper>(a_info)->x(), arc_first_vertex_it<Arc::lower>(b_info)->x()))
    {
      ...
      return FindCrossingPointsResult::disjoint;
    }

    switch (find_arc_crossing_points<Arc::lower, true>(a_info, b_info, callbacks))
    {
    case FindArcCrossingPointsResult::disjoint:
      return FindCrossingPointsResult::disjoint;
    case FindArcCrossingPointsResult::b_side_vertex_inside_a:
      result = FindCrossingPointsResult::b_leftmost_inside_a;
      break;
    case FindArcCrossingPointsResult::b_side_vertex_outside_a:
      result = FindCrossingPointsResult::both_leftmost_outside_other;
      break;
    }
  }
  else
  {
    ...
    switch (find_arc_crossing_points<Arc::lower, false>(b_info, a_info, callbacks))
    {
    case FindArcCrossingPointsResult::disjoint:
      return FindCrossingPointsResult::disjoint;
    case FindArcCrossingPointsResult::b_side_vertex_inside_a:
      result = FindCrossingPointsResult::a_leftmost_inside_b;
      break;
    case FindArcCrossingPointsResult::b_side_vertex_outside_a:
      result = FindCrossingPointsResult::both_leftmost_outside_other;
      break;
  ...
  {
    FindArcCrossingPointsResult arc_result = find_arc_crossing_points<Arc::upper, true>(a_info, b_info, callbacks);
    DIDA_DEBUG_ASSERT(arc_result != FindArcCrossingPointsResult::disjoint);
  }
  else
  {
    FindArcCrossingPointsResult arc_result = find_arc_crossing_points<Arc::upper, false>(b_info, a_info, callbacks);
    DIDA_DEBUG_ASSERT(arc_result != FindArcCrossingPointsResult::disjoint);
  }
